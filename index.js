...
// Owner numbers
const ownerNumber = ["923117298271"];
...
function loadLocalFiles() {
  ...
  if (fs.existsSync('./lib/arslan.html')) {
    ...
    console.log("⚠️ jerry.html not found in lib folder");
  }
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("JEREY-MD~")) {
    console.log("❌ Invalid JERRY-MD session format");
    process.exit(1);
  }
  ...
  console.log("♻️ JERRY-MD session restored successfully");
}
...
// Serve HTML page
app.get('/', (req, res) => {
  const htmlPath = path.join(__dirname, 'lib', 'JERRY.html');
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'JERRY'}</p>
        </div>
      </body>
      </html>
    `);
...
async function connectToWA() {
  ...
  console.log(`🤖 JERRY-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ JERRY-MD Bot connected to WhatsApp!");
      ...
      try {
        ...
        sock.sendMessage(ownerNumber[0] + '923117298271@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
